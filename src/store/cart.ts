...
const MASSES = new Set(["branca", "chocolate", "morango"] as const);
type Massa = "branca" | "chocolate" | "morango";
const TOPO_TYPES = new Set(["nenhum", "simples", "personalizado"]);

function isMassa(v: unknown): v is Massa {
  return typeof v === "string" && MASSES.has(v as Massa);
}
...
function normalizeCartItem(input: unknown): CartItem | null {
  ...
  if (item.kind === "vulcao") {
    ...
    if (!isMassa(item.massa)) return null;
  ...
  persist(
    ...
    }),
    {
      ...
      migrate: (persistedState: unknown) => {
        if (!persistedState || typeof persistedState !== "object") return { items: [] } as Partial<CartState>;
